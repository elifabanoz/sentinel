...
def save_findings(findings, scan_id: str):
    """Bulunan bulgular PostgreSQL'e kaydedilir"""
    if not findings:
        return
...
def process_message(ch, method, properties, body):
    """RabbitMQ'dan gelen scan job'ını işler — hata olursa nack ile geri koyar"""
    try:
        job = json.loads(body)
        scan_id = job["scan_id"]
        target = ScanTarget(url=job["url"], scan_id=scan_id, domain=job["domain"])
        config = ScanConfig(max_requests_per_second=5, request_timeout=10)

        log.info(f"Starting SQLi scan for {target.domain}")
        findings = scanner.scan(target, config)
        log.info(f"Found {len(findings)} SQLi issues")
...
def main():
    log.info("SQLi Scanner worker starting...")
