...
from scanner import DepsScanner
...
def handle_job(job: dict):
    ...
    log.info(f"Starting deps scan for {domain} (scan_id={scan_id})")
    rate_limiter.acquire(domain)

    target = ScanTarget(url=url, scan_id=scan_id, domain=domain)
    config = ScanConfig(max_requests_per_second=2, request_timeout=10)
...
def main():
    log.info("Deps Scanner worker starting...")
